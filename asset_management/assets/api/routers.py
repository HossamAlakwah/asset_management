...
from assets.models import Router, RouterLog
...
class RouterSerializer(StockOnCreateMixin, serializers.ModelSerializer):
    branch_name = serializers.CharField(source="branch.name", read_only=True)
    created_by_username = serializers.CharField(
        source="created_by.username", read_only=True
    )
    logs = RouterLogSerializer(many=True, read_only=True)
...
class RouterViewSet(ExcelIOMixin, viewsets.ModelViewSet):
    queryset = Router.objects.select_related("branch", "created_by").prefetch_related("logs")
    ...
    excel_sheet_name = "Routers"

    ui_title = "Routers"
    ui_group = "Infrastructure"
    ui_icon = "network"
    ui_list_fields = ["serial_number", "model", "status", "ip_address", "location", "branch_name"]
    ui_log_field = "logs"
