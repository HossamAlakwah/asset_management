...
from assets.models import Switch, SwitchLog
...
EXCEL_COLUMNS = [
    ...
    Column("Number Of PoE Ports", "number_of_poe_ports"),
...
class SwitchSerializer(StockOnCreateMixin, serializers.ModelSerializer):
    branch_name = serializers.CharField(source="branch.name", read_only=True)
    created_by_username = serializers.CharField(
        source="created_by.username", read_only=True
    )
    logs = SwitchLogSerializer(many=True, read_only=True)
...
class SwitchViewSet(ExcelIOMixin, viewsets.ModelViewSet):
    queryset = Switch.objects.select_related("branch", "created_by").prefetch_related("logs")
    ...
    excel_sheet_name = "Switches"

    ui_title = "Switches"
    ui_group = "Infrastructure"
    ui_icon = "network"
    ui_list_fields = ["serial_number", "model", "status", "number_of_ports", "number_of_poe_ports", "branch_name"]
    ui_log_field = "logs"
