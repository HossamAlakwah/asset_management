...
from assets.models import NVR, NVRLog
...
EXCEL_COLUMNS = [
    ...
    Column("HDD Capacity", "hdd_capacity"),
...
class NVRLogSerializer(serializers.ModelSerializer):
...
class NVRSerializer(StockOnCreateMixin, serializers.ModelSerializer):
    branch_name = serializers.CharField(source="branch.name", read_only=True)
    created_by_username = serializers.CharField(
        source="created_by.username", read_only=True
    )
    logs = NVRLogSerializer(many=True, read_only=True)
...
class NVRViewSet(ExcelIOMixin, viewsets.ModelViewSet):
    queryset = NVR.objects.select_related("branch", "created_by").prefetch_related("logs")
    ...
    excel_sheet_name = "NVRs"

    ui_title = "NVRs"
    ui_group = "Infrastructure"
    ui_icon = "server"
    ui_list_fields = ["serial_number", "model", "status", "hdd_capacity", "number_of_ports", "branch_name"]
    ui_log_field = "logs"
